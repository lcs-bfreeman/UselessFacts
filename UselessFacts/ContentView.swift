...
struct ContentView: View {
    ...
    func fetchFact() {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            print(String(data: factData, encoding: .utf8)!)

            // Attempt to decode the JSON into an instance of the DadJoke structure
            if let decodedFactData = try? JSONDecoder().decode(Facts.self, from: factData) {

                // DEBUG:
                print("Joke data decoded from JSON successfully")
                print("The joke is: \(decodedFactData.text)")

                // Now, update the UI on the main thread
                DispatchQueue.main.async {

                    // Assign the result to the "someText" stored property
                    someText = decodedFactData.text

                }

            } else {

                print("Could not decode JSON into an instance of the Fact structure.")
